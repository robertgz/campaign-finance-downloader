...
describe('Search by Ballot Item', () => {
  ...
  //   expect(result.results.found === 0 ).toBe(true);
  // }, 50000);
  ...
  //   expect(Array.isArray(result.results.data)).toBe(true);
  //   expect(result.results.found >= 2).toBe(true);
  // }, 50000);

  test.only(`ballotItemSearch with electionDate and ballotItem, should get 18 results`, async () => {
    const result = await ballotItem.ballotItemSearch(urlPrefix, { electionDate: '3/5/2024'});

    outputResult(result);

    expect(Array.isArray(result.results.data)).toBe(true);
    expect(result.results.found >= 18).toBe(true);
  }, 50000);
